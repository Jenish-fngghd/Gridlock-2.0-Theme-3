...
it to the design's two-stage form for a locally-feasible instance classifier:
  ...
  * 2-class MobileNetV3-small (pretrained), same recipe as wrong-side.
...
Run:  python -m src.train.train_seatbelt --epochs 12 --version v1
"""
...
def train(epochs: int, version: str, batch: int, lr: float,
    ...
    model, pretrained = build_model(num_classes=2)
    if freeze_backbone:
        frozen = 0
    ...
    for ep in range(1, epochs + 1):
        ...
        if vm["wrongside_f1"] >= best_f1:
            ...
            torch.save({"state_dict": model.state_dict(), "classes": CLASS_NAMES,
                        "backbone": "mobilenet_v3_small", "input": input_size,
                        "positive_class": "no_seatbelt"}, ckpt_path)
    return {
        "model": "mobilenet_v3_small (pretrained=%s)" % pretrained, "version": version,
...
def main() -> int:
    ...
    append_run_history({"run_id": run_id, "phase": "phase5", "module": "seatbelt",
                        "dataset": "seatbelt-OBB(valid)", "model": "mobilenetv3s-ft",

...
class ConfidenceCascade:
    ...
    def decide(self, vtype: str, confidence: float) -> CascadeDecision:
        ...
        if cal >= auto:
            # High-confidence SAM-3 entity-detection + explicit geometric rule (interpretable
            # basis) -> trust it, no VLM needed. Keeps the (slow/heavy) VLM off the common path.
            band, needs_vlm = AUTO_CONFIRM, False
        elif cal >= floor:
            band, needs_vlm = HUMAN_REVIEW, True   # only the uncertain/low-confidence cases hit VLM
        else:

...
class RoboflowDetector:
    ...
    def _post(self, url_no_key: str, b64: str):
        """POST to a Roboflow model URL (missing only the api_key query param), trying each
        configured key in turn until one returns HTTP 200. Returns the last response object."""
        import requests

        resp = None
        for key in self.api_keys:
            resp = requests.post(f"{url_no_key}&api_key={key}", data=b64,
    ...
    def detect(self, image, violation_type: str) -> dict:
        ...
        if not self.api_keys or cfg is None:
            return {"model_unavailable": True, "fired": False, "confidence": 0.0, "boxes": [],
                    "note": "no api key" if not self.api_keys else f"no model for {violation_type}"}
        try:
            b64 = self._encode(image)
            resp = self._post(f"{_BASE}/{cfg['model']}?confidence=5", b64)
            if resp is None or resp.status_code != 200:
                note = f"HTTP {resp.status_code}: {resp.text[:120]}" if resp is not None else "no response"
                return {"model_unavailable": True, "fired": False, "confidence": 0.0, "boxes": [],
                        "note": note}
    ...
    def detect_helmet(self, image) -> dict:
        ...
        try:
            b64 = self._encode(image)
            resp = self._post(f"{_BASE}/{_HELMET_MODEL}?confidence=5", b64)
            if resp is None or resp.status_code != 200:
                note = f"HTTP {resp.status_code}" if resp is not None else "no response"
                return {"model_unavailable": True, "fired": False, "boxes": [], "note": note}
            preds = resp.json().get("predictions", [])

...
def run(limit: int, variant: str, threshold: float, two_stage: bool = False,
        stage1: str = "small", stage2: str = "rfdetr-large") -> dict:
    try:
    ...
    if two_stage:
        from src.modules.detection import TwoStageDetector
        log(f"[eval_detection] {len(samples)} IDD val | DOCUMENTED two-stage: "
            f"RF-DETR-{stage1} screen -> {stage2} confirm @ thr={threshold}")
        det = TwoStageDetector(stage1_variant=stage1, stage2_model=stage2,
                               confirm_threshold=threshold)
        if det.stage1.model is None:
            return {"error": f"stage1 unavailable: {det.stage1._unavailable_reason}", "model_unavailable": True}
        model_label = f"two-stage(RF-DETR-{stage1}->{det.stage2_kind})"
    else:
    ...
    return {
        "dataset": "IDD-Detection (val)", "images": len(samples), "variant": model_label,
...
def main() -> int:
    ...
    ap.add_argument("--variant", default="nano", help="single-stage variant (if not --two-stage)")
    ap.add_argument("--threshold", type=float, default=0.3)
    ap.add_argument("--two-stage", action="store_true",
                    help="documented cascade: RF-DETR-N/S screen -> RF-DETR-L/Co-DETR confirm (§3.2)")
    ap.add_argument("--stage1", default="small", choices=["nano", "small"])
    ap.add_argument("--stage2", default="rfdetr-large", choices=["rfdetr-large", "codetr"])
    args = ap.parse_args()

    run_id = new_run_id()
    result = run(args.limit, args.variant, args.threshold,
                 two_stage=args.two_stage, stage1=args.stage1, stage2=args.stage2)

...
# `einops` is the only thing actually needed from sam3's [notebooks] extra (vision backbone +
# text encoder construction) — installing that whole extra drags in jupyter/notebook/ipywidgets/
# scikit-image/scikit-learn etc, bloating the image by several GB for nothing this service uses.
RUN python -m venv --system-site-packages /app/.venv-sam3 && \
    /app/.venv-sam3/bin/pip install --upgrade pip && \
    /app/.venv-sam3/bin/pip install -e ./vendor/sam3 einops triton
